...
const WALLPAPER_POPOVER_REVEAL_DURATION_MS = 340
...
const IDLE_MATCHERS = ["hyprctl dispatch dpms off"]
...
function TimerRow({
  ...
  return (
    <box class="wallpaper-timer-card" orientation={Gtk.Orientation.VERTICAL} spacing={6} widthRequest={SCROLLER_WIDTH}>
      <box class="wallpaper-timer-meta" spacing={8} valign={Gtk.Align.CENTER}>
      ...
      <box class="wallpaper-timer-controls" spacing={8} valign={Gtk.Align.CENTER}>
        ...
        <box class="wallpaper-timer-value-box" hexpand halign={Gtk.Align.FILL}>
...
export function WallpaperWidgetButton({ monitor }: { monitor: number }) {
  ...
  const runWallpaperApplyCommand = async (path: string) => {
    const commands = [
      ["swww", "img", "--transition-type", "wipe", "--transition-duration", "0.48", path],
      ["swww", "img", "--transition-type", "outer", "--transition-duration", "0.48", path],
      ["swww", "img", "--transition-type", "simple", "--transition-duration", "0.40", path],
      ["swww", "img", path],
    ]
  ...
  const saveTimer = async (kind: TimerKind) => {
    ...
    const targetLabel = kind === "lock" ? "Hyprlock" : "Hypridle"
  ...
  const wallpapersBusy = createComputed(() => refreshing() || applying())
  ...
  const popoverContent = (
    ...
    >
      ...
      <box class="wallpaper-timers" orientation={Gtk.Orientation.VERTICAL} spacing={10} widthRequest={SCROLLER_WIDTH}>
        <TimerRow
          icon={"󰌾"}
          title="Hyprlock timer"
        ...
        <TimerRow
          icon={"󰒲"}
          title="Hypridle timer"
          subtitle="Display idle timeout"
          value={idleMinutes}
          busy={idleTimerBusy}
  ...
  const popupWindow = (
    <window
      ...
      monitor={monitor}
      namespace={`wallpaper-popup-${monitor}`}
