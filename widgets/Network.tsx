...
const POPOVER_REVEAL_DURATION_MS = 340
...
export function NetworkControl({
  ...
  const refresh = async () => {
    ...
    if (!enabled) {
      ...
      setIcon(current ? wifiSignalIcon(current.signal) : (wgActive ? "󰦝" : "󰖩"))
  ...
  const popoverContent = (
    <box class="network-popover" orientation={Gtk.Orientation.VERTICAL} spacing={10} widthRequest={NETWORK_POPOVER_WIDTH}>
      ...
      <box orientation={Gtk.Orientation.VERTICAL} spacing={6} vexpand>
        ...
        <Gtk.ScrolledWindow vexpand minContentHeight={120} maxContentHeight={187} propagateNaturalHeight>
          <box orientation={Gtk.Orientation.VERTICAL} spacing={4} $={(self) => wifiListBox = self} />
        </Gtk.ScrolledWindow>
      </box>
      ...
      <box orientation={Gtk.Orientation.VERTICAL} spacing={6}>
        ...
        <box orientation={Gtk.Orientation.VERTICAL} spacing={4} $={(self) => wgListBox = self} />
      </box>
  ...
  const popupWindow = (
    <window
      ...
      monitor={monitor}
      namespace={`network-popup-${monitor}`}
  ...
  return (
    <box class="network-shell" valign={Gtk.Align.CENTER} $={(self) => {
      ...
      <button class="network-trigger" valign={Gtk.Align.CENTER} tooltipText={meta} onClicked={togglePopup} $={(self) => {
        trigger = self
